...
bigchart <-  ggplot(data = df_se, aes(x = datum)) +
  ...
  geom_dl(aes(x = datum, y = rolling_average, label = get_label_value(partei)), color = farben[df_se$partei], method = list(dl.trans(x = x + .2, cex = 1.5, family="SZoSansCond-Light"), "last.bumpup")) +
  sztheme_lines +
...
direct.label(bigchart, last.bumpup) 
...
# ggsave(file="data/assets/longterm-poll-article.svg", plot=gt, dpi = 144, units = "in", width = 8.89, height = 5)
